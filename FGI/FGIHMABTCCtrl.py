...
# A control class for the EI3v3JMB strategy written by Justin Michael Boon

class FGIHMABTCCtrl(EI3v3JMB):
	INTERFACE_VERSION = 2
	buy_params = {
		"hma": 20,
		"hma_offset": 10,
	...
	hma_1d_tf = IntParameter(5, 500, default=buy_params["hma"], space="buy", optimize=True)
	hma_1d_IO = CategoricalParameter([True, False], default=buy_params["hma_on"], space="buy", optimize=True)
	# sma_1d_tf = IntParameter(5, 75, default=buy_params["sma"], space="buy", optimize=True)
	hma_1d_offset_tf = IntParameter(1, 200, default=buy_params["hma_offset_switch"], space="buy", optimize=True)
	hma_1d_offset_IO = CategoricalParameter([True, False], default=True, space="buy", optimize=True)
	...
	fgi_data = read_csv('user_data/data/FGI_data/fgi_data.csv', index_col='timestamp', parse_dates=True)
	fgi_fear_on = CategoricalParameter([True, False], default=buy_params["FGIFEAR_on"], space="buy", optimize=True)
	fgi_greed_on = CategoricalParameter([True, False], default=buy_params["FGIGREED_on"], space="buy", optimize=True)
	extreme_greed = CategoricalParameter([70, 75, 80, 85, 90, 95], default=buy_params["GREED"], space="buy", optimize=True)
	extreme_fear = CategoricalParameter([5, 10, 15, 20, 25, 30], default=buy_params["FEAR"], space="buy", optimize=True)
	# print(fgi_data)
	...
	def fetch_FGI(self, dataframe: DataFrame) -> DataFrame:
		...
		dataframe = dataframe.merge(self.fgi_data[['value']], left_index=True, right_index=True, how='left')
		dataframe.rename(columns={'value': 'FGI'}, inplace=True)

		return dataframe
	def info_tf1d_btc_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
		# Indicators
		# -----------------------------------------------------------------------------------------
		# dataframe["sma"] = ta.SMA(dataframe["close"], timeperiod=self.buy_params["sma"])
		dataframe["hma"] = qtpylib.hull_moving_average(dataframe["close"], window=self.buy_params["hma"])
	...
	def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
		...
		btc_info_tf_1d = self.info_tf1d_btc_indicators(btc_info_tf_1d, metadata)  # Call the method here
